# Run CellBender
Cellbender = function(count, cells, total, dim=200, layers=1000, epochs=300){
  command = paste("cellbender remove-background --input", eval(count),
                 "--output output.h5",
                 "--cuda",
...
Doublets = function(count, exp.cells, cell.calls){
...
EnImpute = function(h5, exp.cells, total.drops, z.dim=200, z.layers=1000, ep=300,
