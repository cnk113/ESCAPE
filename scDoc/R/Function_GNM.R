...
dmix <-
  function (x, pars) 
  {
    pars[1] * dgamma(x, shape = pars[2], rate = pars[3]) / (pars[1] * dgamma(x, shape = pars[2], rate = pars[3]) + 
...
get_mix = function(xdata, point, tol=1e-03){
  inits = rep(0, 5)
  ...
  while(eps > tol) {
    ...
    eps = ifelse(is.infinite(loglik), 1e03, (loglik - loglik_old)^2)
    loglik_old = loglik
    iter = iter + 1
